        ...
        m_Scene->GetRegistry().view<TagComponent>().each(
            [&](auto entityHandle, auto &tag)
            {
                ...
                if (entity.HasComponent<PhysicsComponent>())
                {
                    ...
                    // Shape
                    std::string shape =
                        (body->shapeType == AccelEngine::ShapeType::CIRCLE)
                            ? "CIRCLE"
                            : "AABB";
                    ...
                    out << YAML::Key << "inverse_mass" << YAML::Value
                        << body->inverseMass;
                    ...
                    {
                        out << YAML::Key << "half_size" << YAML::Value
                            << YAML::Flow << YAML::BeginSeq
                            << body->aabb.halfSize.x
                            << body->aabb.halfSize.y
                            << YAML::EndSeq;
                    }
                    ...
                    // Constraints
                    out << YAML::Key << "lock_rotation" << YAML::Value
                        << body->lockRotation;

                    out << YAML::Key << "lock_position" << YAML::Value
                        << body->lockPosition;
        ...
        for (auto entityNode : entities)
        {
            try
            {
                ...
                if (auto a = entityNode["Animation"])
                {
                    ...
                    if (!TryGet(a, "texture", path) ||
                        !TryGet(a, "frame_width", fw) ||
                        !TryGet(a, "frame_height", fh) ||
                        !TryGet(a, "frame_count", fn) ||
                        !TryGet(a, "frame_duration", fd) ||
                        !TryGet(a, "looping", loop))
                        continue;
